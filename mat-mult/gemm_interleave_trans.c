...
// float4
// [256,256,1] [1,1,1] 1024x1024x1024 0.220724 s 9.729284 GFLOPS
// [256,256,1] [4,4,1] 1024x1024x1024 0.220724 s 9.729284 GFLOPS
// half4
...
__kernel void gemm_interleaved_transposed_vec4(const int aI_height, // height of aI
   ...
   // const int row = get_global_id(1); // row of aI: [0, m/4) <==> [0, aI_height)
    const int col = get_group_id(0) * 4 + get_local_id(0);
    const int row = get_group_id(1) * 4 + get_local_id(1);
    ...
    for (int p = 0; p < aI_width; p += 8) {
        ...
        c00 += (CL_ELEM_TYPE)aa.s0 * bb;// + (CL_ELEM_TYPE)aa.s0 * bb;
        c10 += (CL_ELEM_TYPE)aa.s1 * bb;// + (CL_ELEM_TYPE)aa.s1 * bb; 
        c20 += (CL_ELEM_TYPE)aa.s2 * bb;// + (CL_ELEM_TYPE)aa.s2 * bb;
        c30 += (CL_ELEM_TYPE)aa.s3 * bb;// + (CL_ELEM_TYPE)aa.s3 * bb;
