...
#ifdef DEBUG
    printf(">>> [DEBUG] cl_build_program_options: %s\n", cl_build_program_options);
#endif
    status = clBuildProgram(*program, 1, device, cl_build_program_options, NULL, NULL);
    if(status != CL_SUCCESS)
    {
        printf(">>> [ERROR] failed to build program: %d\n", (int)status);
        // Error code: -11: syntax error, such as inappropriate ; (may be , is right)
        size_t len;
        clGetProgramBuildInfo(*program, *device, CL_PROGRAM_BUILD_LOG, sizeof(BufferError), BufferError, NULL);
        BufferError[len] = '\0';
